<div align="center">

# Gabriel Ghedin

### Cybersecurity • Security Engineering • DevSecOps • Infrastructure

**Computer Engineering Student | IT Intern @ Eaton**

Building hands-on security labs, infrastructure solutions and automation projects focused on **defensive security, enterprise environments and secure engineering**.

<br>

<a href="https://www.linkedin.com/in/gabriel-ghedin-18a4b02b7">
  <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn">
</a>
&nbsp;
<a href="./assets/Gabriel-Ghedin-Resume.pdf">
  <img src="https://img.shields.io/badge/Resume-View%20PDF-24292F?style=for-the-badge" alt="Resume">
</a>
&nbsp;
<a href="mailto:ghedingabrielaurelio@gmail.com">
  <img src="https://img.shields.io/badge/Email-Contact-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email">
</a>

</div>

---

## 👨‍💻 About Me

I'm a **Computer Engineering student**, graduating in **2027**, currently working as an **IT Intern at Eaton**.

My professional background includes hands-on experience with **technical support, IT infrastructure, enterprise environments, SharePoint, access management, data analysis and process automation**.

These experiences led me to develop a strong interest in **Cybersecurity**, especially in areas where security intersects with infrastructure, identity, automation and software engineering.

I learn by building practical environments that reproduce real-world enterprise scenarios — from **security automation and vulnerability assessment** to **SOC monitoring, Active Directory hardening, vulnerability management and secure CI/CD pipelines**.

- 🎓 **Computer Engineering** — Expected graduation: 2027
- 💼 **IT Intern @ Eaton**
- 🔐 Primary focus: **Cybersecurity & Security Engineering**
- 🛡️ Interested in **SOC / Blue Team & Detection Engineering**
- ⚙️ Interested in **Infrastructure, Automation & DevSecOps**
- 🌎 English: **C1**
- 📈 **Lean Six Sigma Black Belt**

---

## 🎯 Security Focus

<table>
<tr>

<td width="33%" align="center" valign="top">

<h3>🛡️ SOC / Blue Team</h3>

Detection Engineering<br>
SIEM<br>
Incident Investigation<br>
Threat Analysis<br>
Endpoint Monitoring<br>
MITRE ATT&CK

</td>

<td width="33%" align="center" valign="top">

<h3>🔐 Security Engineering</h3>

Infrastructure Hardening<br>
Identity & Access Management<br>
Active Directory Security<br>
Vulnerability Management<br>
Security Automation<br>
Windows Security

</td>

<td width="33%" align="center" valign="top">

<h3>⚙️ DevSecOps</h3>

Secure CI/CD<br>
SAST / SCA<br>
Container Security<br>
GitHub Actions<br>
Software Supply Chain<br>
Security by Design

</td>

</tr>
</table>

<div align="center">

### Security + Infrastructure + Automation

My long-term goal is to work where these three areas intersect.

</div>

---

# 🔐 Featured Security Projects

> Practical projects designed around real-world cybersecurity and enterprise infrastructure scenarios.

<table>

<tr>

<td width="50%" valign="top">

<h3>🔍 Python Vulnerability Scanner</h3>

<b>Python • Networking • Security Automation • GitHub Actions</b>

<br><br>

Safety-first vulnerability assessment tool designed for authorized private laboratory environments.

<b>Key areas:</b>

<br>

• Concurrent TCP port scanning<br>
• Service identification<br>
• Security exposure analysis<br>
• HTTP security-header checks<br>
• Risk scoring<br>
• JSON & HTML reporting<br>
• Automated testing and CI

<br><br>

<a href="https://github.com/GabrielGhedin/python-vulnerability-scanner">
  <b>View Project →</b>
</a>

<br><br>

<b>Release:</b> v1.0.0<br>
<b>Status:</b> ✅ Completed

</td>

<td width="50%" valign="top">

<h3>🛡️ SOC Home Lab</h3>

<b>Wazuh • Sysmon • Windows • Linux • MITRE ATT&CK</b>

<br><br>

Enterprise-inspired Security Operations Center laboratory designed for endpoint telemetry, security monitoring and detection engineering.

<b>Key areas:</b>

<br>

• Security event monitoring<br>
• Windows Sysmon telemetry<br>
• Custom detection rules<br>
• MITRE ATT&CK mapping<br>
• Incident investigation<br>
• Security dashboards

<br><br>

<b>Status:</b> 🚧 Building

</td>

</tr>

<tr>

<td width="50%" valign="top">

<h3>🔒 DevSecOps Secure Pipeline</h3>

<b>GitHub Actions • Docker • CodeQL • Semgrep • Trivy</b>

<br><br>

Secure software delivery pipeline integrating automated security controls throughout the development lifecycle.

<b>Key areas:</b>

<br>

• Static application security testing<br>
• Dependency scanning<br>
• Container security<br>
• Secret detection<br>
• SBOM generation<br>
• Secure CI/CD

<br><br>

<b>Status:</b> 📋 Planned

</td>

<td width="50%" valign="top">

<h3>🏢 Active Directory Security Lab</h3>

<b>Windows Server • Active Directory • PowerShell • GPO</b>

<br><br>

Enterprise Windows laboratory focused on identity management, security policies, monitoring and Active Directory hardening.

<b>Key areas:</b>

<br>

• Active Directory<br>
• Group Policy<br>
• Identity security<br>
• Windows hardening<br>
• Security auditing<br>
• PowerShell automation

<br><br>

<b>Status:</b> 📋 Planned

</td>

</tr>

</table>

---

## ✅ Completed Project Highlights

### 🔍 Python Vulnerability Scanner — v1.0.0

My first completed cybersecurity portfolio project focuses on **safe vulnerability assessment and security automation**.

The project includes:

- Private-lab target validation
- Concurrent TCP scanning
- Basic service identification
- HTTP security analysis
- Risk scoring
- JSON and HTML reporting
- Automated tests
- CI using GitHub Actions
- Automated validation across Python **3.12, 3.13 and 3.14**
- Versioned release management
- Structured project roadmap using GitHub Issues

<a href="https://github.com/GabrielGhedin/python-vulnerability-scanner">
  <b>Explore the repository →</b>
</a>

---

## 🗺️ Cybersecurity Portfolio Roadmap

| Project | Main Focus | Level | Status |
|---|---|---|---|
| 🔍 Python Vulnerability Scanner | Security Automation | Intermediate | ✅ Completed |
| 🛡️ SOC Home Lab | SIEM, Detection Engineering, Incident Analysis | Advanced | 🚧 Building |
| 🔒 DevSecOps Secure Pipeline | CI/CD Security, AppSec, Supply Chain | Advanced | 📋 Planned |
| 🏢 Active Directory Hardening | IAM, Windows Security, GPO | Advanced | 📋 Planned |
| 🚨 Vulnerability Intelligence Dashboard | Vulnerability Management | Intermediate | 📋 Planned |
| ☁️ Microsoft 365 Security Automation | PowerShell, Entra ID, M365 | Intermediate | 📋 Planned |
| 📡 Network Security Monitor | Monitoring, Networking, Observability | Intermediate | 📋 Planned |
| 💻 IT Asset Security Platform | Asset Management, Risk | Advanced | 📋 Planned |
| 🤖 Phishing Detection ML | Machine Learning, Security | Advanced | 📋 Planned |
| 🎫 Help Desk Platform | ITSM, Support, Infrastructure | Intermediate | 📋 Planned |

---

# 🧰 Technical Stack

## 🔐 Security

<kbd>🛡️ Wazuh</kbd>
<kbd>🎯 MITRE ATT&CK</kbd>
<kbd>🔎 Sysmon</kbd>
<kbd>🪟 Windows Security</kbd>
<kbd>🐧 Linux</kbd>
<kbd>🔍 Vulnerability Assessment</kbd>

<br>

## 💻 Development & Automation

<kbd>🐍 Python</kbd>
<kbd>⚡ PowerShell</kbd>
<kbd>🗄️ SQL</kbd>
<kbd>🌐 HTML5</kbd>
<kbd>🎨 CSS3</kbd>
<kbd>🔀 Git</kbd>
<kbd>🐙 GitHub</kbd>

<br>

## ⚙️ Infrastructure & DevSecOps

<kbd>🪟 Windows Server</kbd>
<kbd>🏢 Active Directory</kbd>
<kbd>☁️ Microsoft 365</kbd>
<kbd>📁 SharePoint</kbd>
<kbd>🐳 Docker</kbd>
<kbd>⚙️ GitHub Actions</kbd>

<br>

## 📊 Monitoring & Data

<kbd>📊 Grafana</kbd>
<kbd>🔥 Prometheus</kbd>
<kbd>🐘 PostgreSQL</kbd>

<br>

## 🧪 Engineering Practices

<kbd>Automated Testing</kbd>
<kbd>CI/CD</kbd>
<kbd>Git Flow</kbd>
<kbd>Semantic Versioning</kbd>
<kbd>Technical Documentation</kbd>
<kbd>Security by Design</kbd>

---

# 💼 Professional Experience

## Eaton

### IT Intern

`2026 — Present`

Working in a corporate IT environment with practical exposure to:

- Technical support for hardware, software and end users
- IT infrastructure and enterprise systems
- SharePoint and cloud-based collaboration environments
- Access and permissions management
- Data organization and analysis
- IT process automation
- Information security and governance practices
- Continuous improvement initiatives

My current experience provides the enterprise IT foundation that I am combining with my studies and hands-on cybersecurity projects.

---

## MAHLE

### Engineering Intern — Continuous Improvement

`2025 — 2026`

Worked with continuous improvement, operational analysis and process optimization initiatives.

Highlights:

- Applied structured problem-solving methodologies
- Participated in improvement and internal audit initiatives
- Used data analysis to support operational decisions
- Earned **Lean Six Sigma Black Belt**
- Contributed to process improvement initiatives associated with approximately **R$ 540K in waste reduction**

This experience strengthened my analytical thinking, process-oriented mindset and focus on measurable results.

---

# 🎓 Education

## Bachelor's Degree in Computer Engineering

**Faculdade Santa Lúcia — Mogi Mirim, Brazil**

Expected graduation: **2027**

Current areas of interest:

- Cybersecurity
- Computer Networks
- Operating Systems
- Software Engineering
- Infrastructure
- Automation
- Data Analysis

---

# 🏅 Qualifications

### Lean Six Sigma Black Belt

Applying structured problem-solving, process analysis and continuous improvement principles to technology and automation challenges.

### English

**C1 — Advanced**

Approximately 10 years of English study.

### Additional Studies

<kbd>SQL</kbd>
<kbd>HTML / CSS</kbd>
<kbd>Excel</kbd>
<kbd>Programming Logic</kbd>
<kbd>Infrastructure</kbd>
<kbd>Networking</kbd>
<kbd>Information Security</kbd>

---

# 🛡️ Cybersecurity & Community

### Cybersecurity Outreach — CEBE

Coordinated the team responsible for a cybersecurity presentation designed to bring academic knowledge closer to the community and promote good digital security practices.

This experience helped strengthen my interest in cybersecurity while developing communication, organization and technical presentation skills.

---

# 📚 What I'm Currently Learning

<table>

<tr>

<td width="33%" valign="top">

<h3>🛡️ Cybersecurity</h3>

SOC Operations<br>
Detection Engineering<br>
Vulnerability Management<br>
Active Directory Security<br>
Incident Response<br>
Security Monitoring

</td>

<td width="33%" valign="top">

<h3>⚙️ Engineering</h3>

Python Security Automation<br>
PowerShell<br>
Docker<br>
GitHub Actions<br>
Infrastructure Automation<br>
REST APIs

</td>

<td width="33%" valign="top">

<h3>☁️ Next Steps</h3>

Cloud Security<br>
Microsoft Entra ID<br>
Infrastructure as Code<br>
DevSecOps<br>
Security Engineering<br>
Identity Security

</td>

</tr>

</table>

---

# 🧠 Engineering Principles

<div align="center">

### Security by Design

↓

### Automation

↓

### Observability

↓

### Reproducibility

↓

### Continuous Improvement

<br>

**Secure • Observable • Automated • Reproducible • Resilient**

</div>

---

# 🎯 Current Goals

- Build practical **Blue Team and SOC** experience
- Develop cybersecurity automation tools
- Expand the **Python Vulnerability Scanner** with vulnerability intelligence
- Improve **Windows and Active Directory security** knowledge
- Develop vulnerability management experience
- Build production-style **DevSecOps pipelines**
- Deepen my knowledge of Microsoft security technologies
- Contribute to open-source security projects
- Build a career in **Cybersecurity and Security Engineering**

---

# 💼 Career Interests

I'm particularly interested in opportunities involving:

<br>

<div align="center">

<kbd>🛡️ CYBERSECURITY ANALYST</kbd>
&nbsp;
<kbd>🔎 SOC ANALYST</kbd>
&nbsp;
<kbd>🔐 SECURITY ENGINEER</kbd>
&nbsp;
<kbd>⚙️ DEVSECOPS ENGINEER</kbd>
&nbsp;
<kbd>🐧 INFRASTRUCTURE ENGINEER</kbd>

</div>

<br>

---

# 📫 Let's Connect

I'm always interested in discussing **Cybersecurity, Infrastructure, Automation, DevSecOps and Security Engineering**.

<div align="center">

<a href="https://www.linkedin.com/in/gabriel-ghedin-18a4b02b7">
  <img src="https://img.shields.io/badge/LinkedIn-Gabriel%20Ghedin-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn">
</a>

&nbsp;

<a href="mailto:ghedingabrielaurelio@gmail.com">
  <img src="https://img.shields.io/badge/Email-Contact%20Me-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email">
</a>

&nbsp;

<a href="./assets/Gabriel-Ghedin-Resume.pdf">
  <img src="https://img.shields.io/badge/Resume-View%20PDF-24292F?style=for-the-badge" alt="Resume">
</a>

</div>

---

### 🔒 Portfolio Security Notice

All projects published on this GitHub profile are developed using **personal laboratory environments, synthetic data, public datasets and publicly available information**.

No confidential employer information, internal company configurations, credentials, proprietary source code or sensitive corporate data are published in this portfolio.

---

<div align="center">

### Building security knowledge through real systems.

**Security • Infrastructure • Automation**

<br><br>

📍 Brazil

</div>
